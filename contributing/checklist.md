...
Any component that has text, interacts with text, or lays itself out in relation to text must implement Dynamic Type.
...
1. Set all fonts with a text style enum like `MDCFontTextStyle` or `UIFontTextStyle`, or via `UIFontMetrics`.
...
1. Make sure labels have an appropriate overflow behavior. Usually that will mean setting labels to a `numberOfLines` that is 0.
