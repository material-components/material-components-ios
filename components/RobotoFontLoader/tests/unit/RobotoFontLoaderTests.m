...
- (void)testLightFallbackSystemFonts {
  ...
  XCTAssertEqualObjects(
      ...
      @"The system font must be returned when the fontloader fails to load a font.");
}
...
- (void)testMediumFallbackSystemFonts {
  ...
  XCTAssertEqualObjects(
      ...
      @"The bold system font must be returned when the fontloader fails to load a medium font.");
}
...
- (void)testBoldFallbackSystemFonts {
  ...
  XCTAssertEqualObjects(
      font, [UIFont boldSystemFontOfSize:size],
      @"The bold system font must be returned when the fontloader fails to load a bold font.");
}
...
- (void)testLightItalicFallbackSystemFonts {
  ...
  XCTAssertEqualObjects(
      ...
      @"The italic system font must be returned when the fontloader fails to load an italic font.");
}
...
- (void)testSettingBaseBundleResetsLoader {
  ...
  for (NSUInteger index = 0; index < existingFontLoaders.count; ++index) {
    ...
    XCTAssertNotEqual(
        exisitngFontLoader, newFontLoader,
        @"Fontloader must be new objects when the base bundle gets set.");  // Check that pointers
                                                                            // are different.
  }
}
