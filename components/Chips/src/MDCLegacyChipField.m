...
#import "MDCLegacyChipField.h"

#import "MDCChipView.h"
#import <MDFInternationalization/MDFInternationalization.h>

#import "MDCChipViewDeleteButton.h"
#import "MDCLegacyChipFieldDelegate.h"
#import "MDCTextField.h"
#import "MaterialTextFields.h"

// TODO(cl/315839473): Delete MDCLegacyChipField.m after migrating.

NSString *const MDCLegacyEmptyTextString = @"";
NSString *const MDCLegacyChipDelimiterSpace = @" ";

static const CGFloat MDCLegacyChipFieldHorizontalInset = 15;
static const CGFloat MDCLegacyChipFieldVerticalInset = 8;
static const CGFloat MDCLegacyChipFieldIndent = 4;
static const CGFloat MDCLegacyChipFieldHorizontalMargin = 8;
static const CGFloat MDCLegacyChipFieldVerticalMargin = 8;
static const UIKeyboardType MDCLegacyChipFieldDefaultKeyboardType = UIKeyboardTypeEmailAddress;

const CGFloat MDCLegacyChipFieldDefaultMinTextFieldWidth = 44;
const UIEdgeInsets MDCLegacyChipFieldDefaultContentEdgeInsets = {
    MDCLegacyChipFieldVerticalInset, MDCLegacyChipFieldHorizontalInset,
    MDCLegacyChipFieldVerticalInset, MDCLegacyChipFieldHorizontalInset};

@protocol MDCLegacyChipFieldTextFieldDelegate <NSObject>

- (void)textFieldShouldRespondToDeleteBackward:(UITextField *)textField;

@end

@interface MDCLegacyChipFieldTextField : MDCTextField

@property(nonatomic, weak) id<MDCLegacyChipFieldTextFieldDelegate> deletionDelegate;

@end

@implementation MDCLegacyChipFieldTextField
...
// WARNING: This is a private method, see the warning in MDCLegacyChipField.h.
...
@interface MDCLegacyChipField () <MDCLegacyChipFieldTextFieldDelegate,
                                  MDCTextInputPositioningDelegate,
                                  UITextFieldDelegate>
@end

@implementation MDCLegacyChipField {
...
- (instancetype)initWithFrame:(CGRect)frame {
  ...
  if (self) {
    [self commonMDCLegacyChipFieldInit];

    _chips = [NSMutableArray array];

    MDCLegacyChipFieldTextField *chipFieldTextField =
        [[MDCLegacyChipFieldTextField alloc] initWithFrame:self.bounds];
    ...
    chipFieldTextField.keyboardType = MDCLegacyChipFieldDefaultKeyboardType;
...
- (void)commonMDCLegacyChipFieldInit {
  _chips = [NSMutableArray array];
  _delimiter = MDCLegacyChipFieldDelimiterDefault;
  _minTextFieldWidth = MDCLegacyChipFieldDefaultMinTextFieldWidth;
  _contentEdgeInsets = MDCLegacyChipFieldDefaultContentEdgeInsets;
...
- (void)clearTextInput {
  self.textField.text = MDCLegacyEmptyTextString;
  [self updateTextFieldPlaceholderText];
}
...
#pragma mark - MDCLegacyChipFieldTextFieldDelegate
...
- (void)textFieldDidEndEditing:(UITextField *)textField {
  if ((self.delimiter & MDCLegacyChipFieldDelimiterDidEndEditing) ==
      MDCLegacyChipFieldDelimiterDidEndEditing) {
    if (textField == self.textField) {
      [self commitInput];
    }
  }
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField {
  ...
  if (shouldReturn) {
    [self createNewChipWithTextField:textField delimiter:MDCLegacyChipFieldDelimiterReturn];
  }
...
- (void)textFieldDidChange {
  [self deselectAllChips];
  [self createNewChipWithTextField:self.textField delimiter:MDCLegacyChipFieldDelimiterSpace];
...
- (void)createNewChipWithTextField:(UITextField *)textField
                         delimiter:(MDCLegacyChipFieldDelimiter)delimiter {
  if ((self.delimiter & delimiter) == delimiter && textField.text.length > 0) {
    if (delimiter == MDCLegacyChipFieldDelimiterReturn) {
      [self createNewChipFromInput];
    } else if (delimiter == MDCLegacyChipFieldDelimiterSpace) {
      NSString *lastChar = [textField.text substringFromIndex:textField.text.length - 1];
      if ([lastChar isEqualToString:MDCLegacyChipDelimiterSpace]) {
        [self createNewChipFromInput];
      }
    }
  }
...
- (NSArray<NSValue *> *)chipFramesForSize:(CGSize)size {
  ...
  for (MDCChipView *chip in self.chips) {
    ...
    CGFloat currentOriginY =
        self.contentEdgeInsets.top + (row * (self.chipHeight + MDCLegacyChipFieldVerticalMargin));
    ...
    currentOriginX = CGRectGetMaxX(chipFrame) + MDCLegacyChipFieldHorizontalMargin;
  }
...
- (CGRect)frameForTextFieldForLastChipFrame:(CGRect)lastChipFrame
  ...
  if (availableWidth < desiredTextWidth) {
    ...
    originY += self.chipHeight + MDCLegacyChipFieldVerticalMargin;
    ...
    originX += CGRectGetMaxX(lastChipFrame) + MDCLegacyChipFieldHorizontalMargin;
...
- (CGFloat)textInputDesiredWidth {
  ...
  return MAX(placeholderDesiredWidth,
             CGRectGetWidth(desiredRect) + MDCLegacyChipFieldHorizontalMargin +
                 self.contentEdgeInsets.right + [MDCChipViewDeleteButton imageSideLength]);
}
...
- (UIEdgeInsets)textInsets:(UIEdgeInsets)defaultInsets
  ...
  defaultInsets.left = MDCLegacyChipFieldIndent;
