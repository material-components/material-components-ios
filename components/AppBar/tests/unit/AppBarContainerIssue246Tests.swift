...
class AppBarContainerIssue246Tests: XCTestCase {
  ...
  func testNonNil() {
    ...
    let container = MDCAppBarContainerViewController(contentViewController: content)

    XCTAssertNotNil(container.appBar.headerViewController)
    XCTAssertNotNil(container.appBar.navigationBar)
    XCTAssertNotNil(container.appBar.headerStackView)
  }
}
