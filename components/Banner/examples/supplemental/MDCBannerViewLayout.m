...
@property(nonatomic, readwrite, strong) NSMutableArray *buttonFrames;
@property(nonatomic, readwrite, strong) UIImageView *iconImageView;
@property(nonatomic, readwrite, strong) UILabel *textLabel;

@property(nonatomic, readwrite, assign) CGSize sizeToFit;

@property(nonatomic, readwrite, assign) CGSize frameSize;
@property(nonatomic, readwrite, assign) MDCBannerViewLayoutMode style;
...
- (instancetype)initWithIconImageView:(UIImageView *)iconImageView
  ...
  if (self) {
    ...
    _iconImageView = [iconImageView copy];
    ...
    _buttonFrames = [[NSMutableArray alloc] init];
    for (UIButton *button in buttons) {
      [_buttonFrames addObject:[NSValue valueWithCGRect:button.frame]];
    }
    [self updateLayoutStyleAndFrameSize];
    [self updateLayoutWithStyle:self.style andFrameSize:self.frameSize];
  }
  return self;
}
...
- (void)updateLayoutStyleAndFrameSize {
  ...
  if (self.iconImageView) {
    remainingWidth -= self.iconImageView.frame.size.width;
  ...
  if ([self isAbleToFitTextLabelWithWidthLimit:remainingWidth]) {
    ...
    if (self.iconImageView) {
      [singleLineViews addObject:self.iconImageView];
    }
    ...
    if (self.iconImageView) {
      remainingWidth -= (self.iconImageView.frame.size.width + kSpaceBetweenIconImageAndTextLabel);
      textLabelSize = [self.textLabel sizeThatFits:CGSizeMake(remainingWidth, CGFLOAT_MAX)];
...
- (void)updateLayoutWithStyle:(MDCBannerViewLayoutMode)layoutMode andFrameSize:(CGSize)frameSize {
  switch (layoutMode) {
    case MDCBannerViewLayoutSingleLineStyle: {
      CGFloat currentXAxis = 0.0f;
      currentXAxis += kLeadingPadding;
      if (self.iconImageView) {
        CGRect originalIconImageView = self.iconImageView.frame;
        self.iconImageView.frame =
            CGRectMake(currentXAxis, frameSize.height / 2 - originalIconImageView.size.height / 2,
                       originalIconImageView.size.width, originalIconImageView.size.height);
      ...
      for (NSUInteger index = 0; index < self.buttonFrames.count; ++index) {
        currentXAxis += kButtonHorizontalIntervalSpace;
        CGRect buttonFrame = [self.buttonFrames[index] CGRectValue];
        ...
        self.buttonFrames[index] = [NSValue valueWithCGRect:newButtonFrame];
    ...
    case MDCBannerViewLayoutMultiLineAlignedButtonStyle: {
      ...
      NSUInteger buttonCount = self.buttonFrames.count;
      for (NSUInteger index = 0; index <= buttonCount - 1; ++index) {
        CGRect buttonFrame = [self.buttonFrames[buttonCount - index - 1] CGRectValue];
        currentXAxis -= buttonFrame.size.width;
        CGRect newButtonFrame =
            CGRectMake(currentXAxis, currentYAxisCenter - buttonFrame.size.height / 2,
                       buttonFrame.size.width, buttonFrame.size.height);
        self.buttonFrames[buttonCount - index - 1] = [NSValue valueWithCGRect:newButtonFrame];
        currentXAxis -= kButtonHorizontalIntervalSpace;
      }
      break;
    }
...
- (CGFloat)buttonsWidthSum {
  ...
  for (NSValue *buttonFrame in self.buttonFrames) {
    buttonsWidthSum += [buttonFrame CGRectValue].size.width;
  }
  if (self.buttonFrames.count > 0) {
    buttonsWidthSum += (self.buttonFrames.count - 1) * kButtonHorizontalIntervalSpace;
  }
...
- (CGFloat)buttonsHeightSum {
  ...
  if (self.buttonFrames.count > 0) {
    buttonsHeightSum += (self.buttonFrames.count - 1) * kButtonVerticalIntervalSpace;
  }
...
- (CGRect)iconImageViewFrame {
  return self.iconImageView.frame;
}
