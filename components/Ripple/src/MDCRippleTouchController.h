...
/**
 ...
 Note: If YES is passed for the deferred parameter and the -[MDCRippleTouchControllerDelegate
 rippleTouchController:insertRippleView:intoView:] is *not* implemented the rippleview will be
 automatically added as top subview to the given view if the first on tap event is happening. In
 case -[MDCRippleTouchControllerDelegate rippleTouchController:insertRippleView:intoView:] *is*
 implemented, it's the responsibility of the delegate to add the ripple view in the proper position
 within view's hierarchy if the delegate method is called.
