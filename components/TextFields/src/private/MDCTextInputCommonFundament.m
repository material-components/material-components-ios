...
- (void)updatePlaceholderToOverlayViewsPosition {
  ...
  if (textField.leadingView.superview) {
    ...
    if ([self.textInput.positioningDelegate respondsToSelector:@selector(leadingViewTrailingPaddingConstant)]) {
      leadingViewTrailingConstant = [self.textInput.positioningDelegate leadingViewTrailingPaddingConstant];
    }
