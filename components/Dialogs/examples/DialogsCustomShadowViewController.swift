...
class CustomShadowViewController: UIViewController {
  ...
  override func viewDidLoad() {
    ...
    bodyLabel.text =
      "This presented view has a custom colored shadow so we've disabled the default shadow."
...
class DialogsCustomShadowViewController: UIViewController {
  ...
  @objc func tap(_ sender: Any) {
    ...
    // We disable the default shadow that is implemented via the trackingView in the
    // presentation controller.
    if let presentationController = presentedController.mdc_dialogPresentationController {
      presentationController.isShadowHidden = true
    }
  }

  // MARK: Catalog by convention
  @objc class func catalogBreadcrumbs() -> [String] {
    return [ "Dialogs", "Custom Shadowed Dialog"]
  }
