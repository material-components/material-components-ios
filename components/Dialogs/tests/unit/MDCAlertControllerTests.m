...
- (void)testMDCAdjustsFontForContentSizeCategoryScalesCustomNonFontScalerFont {
  ...
  if (actionButton.enableTitleFontForState) {
    ...
    XCTAssertEqualObjects(actionButton.titleLabel.font.fontName, baseFont.fontName);
...
- (void)testMDCAdjustsFontForContentSizeCategoryUsesFontScalerFonts {
  ...
  if (actionButton.enableTitleFontForState) {
    ...
    XCTAssertEqualObjects(actionButton.titleLabel.font.fontName, scaledFont.fontName);
  }
}
  ...
  if (@available(iOS 10.0, *)) {
    ...
    CGFloat expectedPointSize =
        (CGFloat)CustomScalingCurve()[UIContentSizeCategoryAccessibilityExtraExtraExtraLarge]
            .doubleValue;
    XCTAssertEqualWithAccuracy(alert.alertView.messageLabel.font.pointSize, expectedPointSize,
                               0.001);
    XCTAssertEqualWithAccuracy(alert.alertView.titleLabel.font.pointSize, expectedPointSize, 0.001);
    if (actionButton.enableTitleFontForState) {
      XCTAssertEqualWithAccuracy([actionButton titleFontForState:UIControlStateNormal].pointSize,
                                 expectedPointSize, 0.001);
    } else {
      XCTAssertEqualWithAccuracy(actionButton.titleLabel.font.pointSize, scaledFont.pointSize,
                                 0.001);
    }
  }
}
...
- (void)testMDCAdjustsFontForContentSizeCategoryDownscalesFontScalerFontsWithLocalTraitCollection {
  if (@available(iOS 10.0, *)) {
    ...
    if (actionButton.enableTitleFontForState) {
      ...
      XCTAssertEqualWithAccuracy(actionButton.titleLabel.font.pointSize, expectedPointSize, 0.001);
    }
  }
}
