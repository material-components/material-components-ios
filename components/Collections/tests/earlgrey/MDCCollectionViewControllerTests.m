...
+ (void)scrollToElementWithIdentifier:(nonnull NSString *)identifier
              inElementWithIdentifier:(nonnull NSString *)scrollableIdentifier
                            direction:(GREYDirection)direction
                             withStep:(NSInteger)step
                                  tap:(BOOL)tap {
  id<GREYMatcher> targetMatcher = grey_allOfMatchers(@[grey_accessibilityID(identifier),
...
+ (void)returnToMainScreen {
  ...
  [[EarlGrey selectElementWithMatcher:grey_accessibilityID(@"back_bar_button")] performAction:grey_tap()];
...
+ (GREYActionBlock *)getBounds:(CGRect *)bounds {
  return [GREYActionBlock actionWithName:@"getBoundsFromUIView" constraints:grey_respondsToSelector(@selector(bounds)) performBlock:^BOOL(id element, NSError *__strong *errorOrNil) {
...
- (GREYAssertionBlock *)assertBoundsWidthLessThanOrEqualTo:(CGFloat)width {
  return [GREYAssertionBlock assertionWithName:@"boundsAreGreaterOrEqualThanOtherBounds"
                       assertionBlockWithError:^BOOL(id element, NSError *__strong *errorOrNil) {
                         ...
                         XCTAssertLessThanOrEqual(asView.bounds.size.width,
                                                  width,
...
- (void)testEachSectionWidthLessOrEqualThanSectionAbove {
  [MDCCollectionViewControllerTests jumpToExampleWithBreadcrumbs:@[@"Collections", @"Custom Section Insets"]];

  // Get the number of sections in the example collection view
  NSInteger totalSections;
  [[EarlGrey selectElementWithMatcher:grey_accessibilityID(@"collectionsCustomSectionInsetsCollectionView")]
  ...
  for (int section = 0; section < totalSections; ++section) {
    ...
    [MDCCollectionViewControllerTests scrollToElementWithIdentifier:cellIdentifier
                                            inElementWithIdentifier:@"collectionsCustomSectionInsetsCollectionView"
     direction:kGREYDirectionDown
                                                           withStep:-1
                                                                tap:NO];
    [MDCCollectionViewControllerTests scrollToElementWithMatcher:grey_allOfMatchers(@[grey_kindOfClass([UISwitch class]),
                                                                                      grey_ancestor(grey_accessibilityID(cellIdentifier)),
                                                                                      grey_sufficientlyVisible(),
                                                                                      grey_interactable()])
                                            inElementWithMatcher:grey_accessibilityID(@"collectionsCustomSectionInsetsCollectionView")
                                                       direction:kGREYDirectionDown withStep:-1 action:grey_turnSwitchOn(YES)];
